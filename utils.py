...
async def maybe_download_and_extract_tarball(
    ...
    if os.path.exists(destination_path) and os.listdir(destination_path):
        print(
            f"Files already present in the `{destination_path}`, nothing will be downloaded."
        )
...
async def maybe_download(
    ...
    if remote_path:
        ...
        if remote_path.endswith(".tar"):
            ...
            assert (
                remote_filenames is not None
            ), "remote_filenames must be provided if `remote_path` is not a tarball"
        ...
        raise NotImplementedError(
            "Downloading from HF not implemented yet, please provide a remote path to a flat tarball containing your model artifacts."
        )
...
def init_model_config() -> dict:
    ...
    if "is_instruct" not in config:
        print(
            "`is_instruct` not specified, attempting to infer from `model_id` and `model_url`."
        )
